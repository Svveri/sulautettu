...
uint32_t red_time_us = 0;
uint32_t green_time_us = 0;
uint32_t yellow_time_us = 0;
...
static void dispatcher_task(void *unused1, void *unused2, void *unused3)
{
	while (true) {
		...
		uint32_t dispatcher_time_us = k_cyc_to_us_floor32(end- start);
		printk("Dispatcher task time: %u us\n", dispatcher_time_us);
...
void green_task(void *, void *, void*) {
	while (true) {
		...
		uint32_t end = k_cycle_get_32();
		green_time_us = k_cyc_to_us_floor32(end- start);
		//printk("Green task time: %u us\n", green_time_us);
...
void yellow_task(void *, void *, void*) {
	while(true){
		...
		gpio_pin_set_dt(&green,0);
		uint32_t end = k_cycle_get_32();
		yellow_time_us = k_cyc_to_us_floor32(end- start);
		//printk("Yellow task time: %u us\n", yellow_time_us);
		k_sem_give(&release_sem);
	}
}
...
void red_task(void *, void *, void*) {
	while (true) {
		...
		gpio_pin_set_dt(&red, 0);
		uint32_t end = k_cycle_get_32();
		red_time_us = k_cyc_to_us_floor32(end- start);
		//printk("Red task time: %u us\n", red_time_us);
		// semafori
		k_sem_give(&release_sem); 
		// Sleep nor yield not needed
		// k_yield();
